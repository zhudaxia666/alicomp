...
def Test_data():
#将sen2的数据按照2轴方向把s2[i,j,:,k]32元素的ndarray拼接成一维数组
...
def get_all_data(counts, tr_s2):
    #连续取sen2的counts个数据，放入res_list列表里，res_list元素也列表（包含10个32*32元素的列表
...
def onehot_tranIndex(label):
    #通过label求出每个label元素对应的类别索引，然后组成ndarray
...
def result(train_sen2, train_label):
    ...
    train_data = get_all_data(10000,tr_s2).reshape(10000, -1)
    # print(train_data.shape) #(8000, 18, 32, 32)
    val_data = get_all_data(8000, te_s2).reshape(8000, -1)
...
csvfile = open('new_accuracy.csv', 'w', newline='')
write1 = csv.writer(csvfile)
for i in one_hots:
    write1.writerow(i)
csvfile.close()
