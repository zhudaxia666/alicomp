...
def result(train_sen2, train_label):
    ...
    train = random.sample(range(len(train_label)), 30000)
    ...
    train_data = get_all_data(30000,tr_s2).reshape(30000, -1)
    # print(train_data.shape) #(8000, 18, 32, 32)
    val_data = get_all_data(4000,te_s2).reshape(4000, -1)

    train_label = get_label(30000, tr_label)
...
csvfile = open('new_accuracy_train_sen2_1_6.csv', 'w', newline='')
write1 = csv.writer(csvfile)
for i in one_hots:
    write1.writerow(i)
csvfile.close()
