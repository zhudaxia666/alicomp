...
def xgboost_class(train_data,train_label,valid_data,valid_label,test_data):
    ...
    'max_depth':10, # 构建树的深度 [1:],典型值：3-10
    #'lambda':450,  # L2 正则项权重
...
file=h5py.File("new_train1.h5",'r')
...
#     train=random.sample(range(125623),8000)
#     test=list(set(range(125623)).difference(set(train)))
#     # test=list(np.delete(range(125623),train))
#     test=random.sample(test,5000)
...
#     print("第%d训练完" % (j+1))
#     del train,test,train_data,tr_s1,tr_s2,te_s1,te_s2,te_label,tr_label,val_data,val_label,test_data
#     gc.collect()
    # print("第%d训练完" % (j+1))
tr_s1=[]
tr_s2=[]
...
for j in range(20):
    ...
    train=random.sample(range(136073),10)
    print(type(train),train)
    test=list(set(range(136073)).difference(set(train)))
    # test=list(np.delete(range(125623),train))
    test=random.sample(test,5)
    print(type(test),test)

    for i in range(len(train)):
        tr_s1.append(train_sen1[train[i]])
        tr_s2.append(train_sen2[train[i]])
        tr_label.append(train_label[train[i]])

    for k in range(len(test)):
        te_s1.append(train_sen1[test[k]])
        te_s2.append(train_sen2[test[k]])
        te_label.append(train_label[test[k]])

    train_data=get_all_data(10,tr_s1,tr_s2).reshape(10,-1)
    # print(train_data.shape) #(8000, 18, 32, 32)
    val_data=get_all_data(5,te_s1,te_s2).reshape(5,-1)

    train_label=get_label(10,tr_label)
    train_label=onehot_tranIndex(train_label)
    val_label=get_label(5,te_label)
    val_label=onehot_tranIndex(val_label)
...
# csvfile.close()
# print("预测完毕!")
# train_s1=np.array(tr_s1).reshape(8000,-1)
# train_s2=np.array(tr_s2).reshape(8000,-1)
# train_label=np.array(tr_label).reshape(8000,-1)

# test_s1=np.array(te_s1).reshape(5000,-1)
# test_s2=np.array(te_s2).reshape(5000,-1)
# test_label=np.array(te_label).reshape(5000,-1)
